###### tags: `Offsec` `PG Play` `Easy` `Linux` # Blogger
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check `http://192.168.170.217/assets/fonts/blog/` and then use `Wappalyzer` to find it is `wordpress`, use `wpscan`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Found on google [ CVE-2020-24186](https://github.com/hev0x/CVE-2020-24186-wpDiscuz-7.0.4-RCE/blob/main/wpDiscuz_RemoteCodeExec.py), first add `blogger.pg` to `/etc/hosts`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ sudo nano /etc/hosts 

192.168.170.217 blogger.pg
``` Click any page at `http://blogger.pg/assets/fonts/blog/` and find a comment section at the bottom. Note the parameter `?p=1`, you can use the previous payload to get a reverse shell.
```
...
``` Start nc, refer to [Perl no sh](https://www.revshells.com/) to get a reverse shell to kali. After geting the shell, go to `/home/james` to get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Use [CVE-2021-4034](https://github.com/joeammond/CVE-2021-4034/blob/main/CVE-2021-4034.py) to get root, then go to /root to get proof.txt
```
