###### tags: `Offsec` `PG Play` `Intermediate` `Linux` # DC-9
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.243.209/search.php` and refer to [Manual vs automatic testing for SQL injection](https://belcyber.medium.com/manual-vs-automatic-testing-for-sql-injection-cf043c6f0dd1)
```sql
# search
...
Phone No: 5
Email: 6
``` Confirmed there are 6 columns, now find `hostname` and `db version`
```sql
...
Search results
Email: UserDetails
``` Find column names in the table
```sql
# Table "Users"
...
``` You can use `admin/transorbital1` to log in to `http://192.168.243.209/manage.php`
LFI
```
...
``` Use [Port Knocking attack](https://github.com/eliemoutran/KnockIt) 22port,rustscan
```
...
``` ssh login with `chandlerb/UrAG0D!`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Use [CVE-2021-3156](https://github.com/worawit/CVE-2021-3156?tab=readme-ov-file) to get root, then get proof.txt in /root
```
...
``` Also get local.txt in `/home/fredf`
```
