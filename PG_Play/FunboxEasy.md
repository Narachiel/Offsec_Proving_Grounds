###### tags: `Offsec` `PG Play` `Easy` `Linux` # FunboxEasy
```
┌──(kali㉿kali)-[~/pgplay]
...
``` busterscan,scanget `192.168.211.111/store`, has `http://192.168.211.111/store/database/readme.txt.txt`insidehas accountpassword `admin/admin`
```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` via Google Search CSE bookstore can find [edb-47887](https://www.exploit-db.com/exploits/47887); you can get a shell and upload a reverse shell
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `/home/tony`has a file`password.txt`
```
www-data@funbox3:/home/tony$ cat password.txt
ssh: yxcvbnmYYY
gym/admin: asdfghjklXXX
/store: admin@admin.com admin
``` usesshlogintony's account,`sudo -l`afterCheck[GTFOBins](https://gtfobins.github.io/gtfobins/pkexec/#sudo)'s pkexec, can get root shell
```
┌──(kali㉿kali)-[~/pgplay]
...
# whoami
root
``` in `/root` get proof.txt, in `/var/www` get local.txt
```
