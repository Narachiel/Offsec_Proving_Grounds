###### tags: `Offsec` `PG Play` `Easy` `Linux` # PyExp
```
┌──(kali㉿kali)-[~/pgplay]
...
+--------------------------------------------------------------------------------------------------------------------------+----------------------------------------------+
``` Search[Fernet (Decode)](https://asecuritysite.com/tokens/ferdecode)
```
Token:	
...
Date created:	Mon Aug 10 21:02:44 2020
``` use`lucy`and```wJ9`"Lemdv9[FEw-```login，in `/home/lucy`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` wait for reverse shellget root，get proof.txt in /root
```
