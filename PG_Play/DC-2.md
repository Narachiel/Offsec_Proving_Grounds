###### tags: `Offsec` `PG Play` `Easy` `Linux` # DC-2
```
┌──(kali㉿kali)-[~/pgplay]
...
``` First add `http://dc-2/` to `/etc/hosts`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ sudo nano /etc/hosts 

192.168.220.194 dc-2
``` Check the `flag` page at `http://dc-2/index.php/flag/` ![DC-2_1.png](picture/DC-2_1.png) It says to use `cewl` to generate a wordlist
```
┌──(kali㉿kali)-[~/pgplay]
└─$ cewl -w pass.txt http://dc-2/ 
``` First use `wpscan` to enumerate
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Found users `admin`, `jerry`, and `tom`. Put them into `user.txt` and use `wpscan` to brute-force
```
┌──(kali㉿kali)-[~/pgplay]
└─$ cat user.txt         
admin
...
``` get `jerry`password`adipiscing`,and`tom`password`parturient`, use`tom`accountlogin,find `rbash`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` First use`compgen -c`Checkhas canuse,findcanuse`vi` Check[GTFOBins](https://gtfobins.github.io/gtfobins/vi/#shell) use can in `/home/tom`get local.txt
```
tom@DC-2:~$ compgen -c
...
wait
...
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/git/#sudo),use afterget root,get proof.txt in /root
```
