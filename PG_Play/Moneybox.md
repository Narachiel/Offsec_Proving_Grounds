###### tags: `Offsec` `PG Play` `Easy` `Linux` # Moneybox
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ftpobtain `trytofind.jpg`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.195.230 

ftp> get trytofind.jpg
``` scan80port, `http://192.168.195.230/blogs/`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ![Moneybox_1.png](picture/Moneybox_1.png) F12 `the hint is the another secret directory is S3cr3t-T3xt` `http://192.168.195.230/S3cr3t-T3xt/` ![Moneybox_2.png](picture/Moneybox_2.png) has `Secret Key 3xtr4ctd4t4`,extractdata use`steghide`
```
...
``` use`hydra`,get password`987654321`
```
┌──(kali㉿kali)-[~/pgplay/Moneybox]
...
``` sshlogin`renu`'s account,canin `/home/renu`get local.txt
```
┌──(kali㉿kali)-[~/pgplay/Moneybox]
...
``` in kali usessh,can login`lily`'s account
```
┌──(kali㉿kali)-[~/pgplay/Moneybox]
...
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/perl/#sudo),get root can get proof.txt in /root
```
