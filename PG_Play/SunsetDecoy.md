###### tags: `Offsec` `PG Play` `Easy` `Linux` # SunsetDecoy
```
┌──(kali㉿kali)-[~/htb]
...
``` Check`80 port`,can a file`save.zip`,Try usejohn crackpassword ![SunsetDecoy_1.png](picture/SunsetDecoy_1.png) ```
...
Session completed.
``` The extracted files are: ![SunsetDecoy_2.png](picture/SunsetDecoy_2.png) Check`shadow file`
```
## shadow
...
``` Check`passwd`,has a account `296640a3b825115a47b68fc44501c828`
```
...
Press 'q' or Ctrl-C to abort, almost any other key for status
server           (?) 
``` sshlogin`296640a3b825115a47b68fc44501c828`account,finduse`rbash`
```
┌──(kali㉿kali)-[~/pgplay/SunsetDecoy]
...
``` modifyusebashlogin,CheckPATHfind in home,Change to`export PATH=$PATH:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin`,can in `/home/296640a3b825115a47b68fc44501c828`get local.txt
```
┌──(kali㉿kali)-[~/pgplay/SunsetDecoy]
...
``` Try[CVE-2021-3156](https://github.com/worawit/CVE-2021-3156?tab=readme-ov-file), root,in /rootcan get proof.txt
```
