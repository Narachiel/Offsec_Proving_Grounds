###### tags: `Offsec` `PG Play` `Easy` `Linux` # CyberSploit1
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check `192.168.215.92` and press F12 to find a `username:itsskv`
Then check `192.168.215.92/robots` and find a base64 string, decode it to find the password
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ssh login, find local.txt in `/home/itsskv`
```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` Try [CVE-2021-4034](https://github.com/ly4k/PwnKit), get `proof.txt` in /root
```
