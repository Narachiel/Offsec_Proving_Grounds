###### tags: `Offsec` `PG Play` `Easy` `Linux` # Amaterasu
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check `http://192.168.243.249:33414/file-list?dir=/home/`
```
0	"alfredo"
``` Check `http://192.168.243.249:33414/file-list?dir=/home/alfredo/`
```
0	".bash_logout"
1	".bash_profile"
2	".bashrc"
...
5	"restapi"
6	".bash_history"
``` You can upload `authorized_keys`, but it says only `txt` and similar files are allowed. See [Upload files with CURL](https://medium.com/@petehouston/upload-files-with-curl-93064dcccc76)
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ssh login, get local.txt in `/home/alfredo`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check `/usr/local/bin/backup-flask.sh`
```bash
...
``` Use [Wildcard Injection](https://medium.com/@silver-garcia/how-to-abuse-tar-wildcards-for-privilege-escalation-tar-wildcard-injection-612a6eac0807) or [Linux Privilege Escalation: Wildcards with tar](https://medium.com/@polygonben/linux-privilege-escalation-wildcards-with-tar-f79ab9e407fa)
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Wait for reverse shell, get proof.txt in /root
```
sh-5.1# whoami
root
sh-5.1# cd /root
