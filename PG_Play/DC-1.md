###### tags: `Offsec` `PG Play` `Easy` `Linux` # DC-1
```
┌──(kali㉿kali)-[~/pgplay]
...
``` searchsploitfindDrupal's,find has [edb-44449](https://www.exploit-db.com/exploits/44449)can use
```
┌──(kali㉿kali)-[~/pgplay]
...
DC-1>> 
``` can get a shell; try uploading a reverse shell
```
┌──(kali㉿kali)-[~/pgplay]
...
DC-1>> ./shell_9001
``` in the /home directory you can get local.txt's flag
```
cat local.txt
75c0eb217d6c430df980febe12b51851
``` `/var/www`has flag1.txt
```
cat flag1.txt
Every good CMS needs a config file - and so do you.
``` There is flag4.txt in the `/home` directory
```
cat flag4.txt
Can you use this same method to find or access the flag in root?

Probably. But perhaps it's not that easy.  Or maybe it is?
``` use find to find binaries
```
...
/sbin/mount.nfs
``` Search[GTFOBins](https://gtfobins.github.io/gtfobins/find/#shell), get root; after that, in /root you can get proof.txt
```
