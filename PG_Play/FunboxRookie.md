###### tags: `Offsec` `PG Play` `Easy` `Linux` # FunboxRookie
```
┌──(kali㉿kali)-[~/pgplay]
...
``` useftplogin,I `tom.zip`'s and file,downloadtom.zip,usejohncrackget password`iubire`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.215.107
ftp> get tom.zip
...
``` fileafterget `id_rsa`, use`tom`and`id_rsa` sshlogin,in `/home/tom`get local.txt
```
┌──(kali㉿kali)-[~/pgplay/FunboxRookie]
└─$ chmod 600 id_rsa
...
``` findhas a `.mysql_history`file,can tom's password `xx11yy22!`
```
tom@funbox2:~$ ls -al
total 40
...
quit
``` use`sudo -l` password`xx11yy22!`,find has use, `sudo su`in `/root`pathget proof.txt
```
tom@funbox2:~$ sudo -l
