###### tags: `Offsec` `PG Play` `Easy` `Linux` # SunsetNoontide
```
┌──(kali㉿kali)-[~/pgplay]
...
``` googleSearchget [UnrealIRCd-3.2.8.1-Backdoor
](https://github.com/Ranger11Danger/UnrealIRCd-3.2.8.1-Backdoor/blob/master/exploit.py),modifyinside's `local_ip`and`local_port`
```
# Sets the local ip and port (address and port to listen on)
local_ip = '192.168.45.158'  # CHANGE THIS
local_port = '9001'  # CHANGE THIS 
``` Start ncexecute`exploit.py`,in `/home/server`find local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` via`su root`can root,in /rootget root.txt
```
