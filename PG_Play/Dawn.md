###### tags: `Offsec` `PG Play` `Easy` `Linux` # Dawn
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `http://192.168.217.11/logs/` can download `management.log`, check inside to see what is executed
```
...
``` Create a new file `product-control`, put it on smb and wait for it to be executed, then wait for reverse shell
```
## product-control
...
``` reverse can in `/home/dawn`get local.txt
```
...
``` find binary, Search[GTFOBins](https://gtfobins.github.io/gtfobins/zsh/#suid)
get rootafterget proof.txt in /root
```
