###### tags: `Offsec` `PG Play` `Easy` `Linux` # OnSystemShellDredd
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ftplogin,`ls -al`Check has a `.hannah`directory,enterget a `id_rsa`file
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.215.130 
...
``` sshloginhannah's account,`/home/hannah`get local.txt
```
...
``` findbinaries,Use [GTFOBins](https://gtfobins.github.io/gtfobins/cpulimit/#suid),in `root`find proof.txt
```
