###### tags: `Offsec` `PG Play` `Easy` `Linux` # Empire-breakout
```
┌──(kali㉿kali)-[~/pgplay]
...
``` F12`192.168.215.238:80`page,has ```
don't worry no one will get here, it's safe to share with you my access. Its encrypted :)

++++++++++[>+>+++>+++++++>++++++++++<<<<-]>>++++++++++++++++.++++.>>+++++++++++++++++.----.<++++++++++.-----------.>-----------.++++.<<+.>-.--------.++++++++++++++++++++.<------------.>>---------.<<++++++.++++++.
``` [dCode](https://www.dcode.fr/cipher-identifier)find `Trithemius Cipher`,[decode](https://www.dcode.fr/trithemius-cipher)afterget `.2uqPEfj3D<P'a-3` use`enum4linux` ```
┌──(kali㉿kali)-[~/pgplay]
...
``` useaccount`cyber` password`.2uqPEfj3D<P'a-3`can login`https://192.168.215.238:20000/` has a terminalcan start,downloadreverseshell use ![Empire-breakout_1.png](picture/Empire-breakout_1.png) ```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` tar has get file's, Check[GTFOBins](https://gtfobins.github.io/gtfobins/tar/#file-read). I can use tar to get `/var/backups/.old_pass.bak`, can obtain password; root password can get root; in /root can get proof.txt
```
