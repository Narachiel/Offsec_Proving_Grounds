###### tags: `Offsec` `PG Play` `Hard` `Linux` # InsanityHosting
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.163.124/news/`insidehas has user `Otis`, use`hydra`crackcrack `http://192.168.163.124/monitoring/login.php`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Try `otis`andpassword `123456`can login`http://192.168.163.124/monitoring/login.php`and`http://192.168.163.124/webmail/src/login.php`
login`monitoring dashboard`after,canfind can`add new`,new testafter, in webmail ```
...
``` canTryTry`sql injection`!!,in test Change to
```sql
admin" UNION SELECT @@hostname,@@version,NULL,NULL;--+
``` ![InsanityHosting_1.png](picture/InsanityHosting_1.png) `webmail` ![InsanityHosting_2.png](picture/InsanityHosting_2.png) ```sql
...
``` via[sql injection](https://book.hacktricks.xyz/pentesting-web/sql-injection#extract-database-names-table-names-and-column-names) get `database`
```sql
# monitoring
...
``` Refer to a [SQL Injections # 2 Union based SQLI](https://notchxor.github.io/oscp-notes/2-web/sqli/)
...
``` `Get all columns from a specific table` `users`
```sql
# monitoring
...
``` sshlogin,in `/home/elliot`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
