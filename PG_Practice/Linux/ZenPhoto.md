###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # ZenPhoto
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `robots`,`http://192.168.182.41/test/robots`
```
User-agent: *
...
``` Check`http://192.168.182.41/test/zp-core/`findlogin,canfuff
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.182.41/test/zp-core/htaccess`,`1.4.1`
```
...
...
``` Search[edb-18083](https://www.exploit-db.com/exploits/18083),downloaduse,get shell,canreverse
```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` Search[edb-15285](https://www.exploit-db.com/exploits/15285),useget root,get proof.txt in /root
```
┌──(kali㉿kali)-[~/pgplay]
