###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Boolean
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.221.231`login,Click`Create account` ![Boolean_1.png](picture/Boolean_1.png) loginlogin,Firstopen`burpsuite``edit`
```json
# request
...
``` Click`Boolean`can`File Manager`,tryuploadphp reverseshell,find`http://192.168.221.231/?cwd=&file=shell.php&download=true`pathLFI,downloadpasswd
```
...
```
keysid_rsause,upload`authorized_key`
```
Filename 	        Size 	    Date
authorized_keys 	563 Bytes 	19 May 2024 23:39
known_hosts 	    222 Bytes 	25 Oct 2022 09:58
keys 	            4 KB 	    25 Oct 2022 09:58
``` usesshlogin,`/home/remi`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `/home/remi/.ssh/keys`find`root``id_rsa`key,shellusesshlogin,tryFirstreverse shellssh
```
┌──(kali㉿kali)-[~/pgplay]
