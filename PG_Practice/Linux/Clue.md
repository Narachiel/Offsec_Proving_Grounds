###### tags: `Offsec` `PG Practice` `Hard` `Linux` # Clue
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.181.240:3000/`,google[edb-49362](https://www.exploit-db.com/exploits/49362)canRefer touse
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`cassie`ssh,Check[edb-47799](https://www.exploit-db.com/exploits/47799)PASSWORD`SecondBiteTheApple330`,fail
```
ADDRESS=sys.argv[1]
CMD=sys.argv[2]
...
``` smb,google[How to change FreeSWITCH event socket password?](https://inextrix.atlassian.net/wiki/spaces/ASTPP/pages/5572241/How+to+change+FreeSWITCH+event+socket+password)path`/usr/local/freeswitch/conf/autoload_configs/event_socket.conf.xmll`,/etc..,tryuse`49362.py`
```
┌──(kali㉿kali)-[~/pgplay]
...
Authenticated
``` login,`/var/lib/freeswitch/`local.txt
```
...
``` canthen use get `cassie`'s password,Check`sudo -l`,`/usr/local/bin/cassandra-web`to seecansudo`cassandra-web`
```
...
``` useCheck`anthony`directory,usegetcassieshellcurl,curl`anthony`directory,findroot,`id_rsa`
```
...
``` login,findproof`proof_youtriedharder.txt`
```
┌──(kali㉿kali)-[~/pgplay]
