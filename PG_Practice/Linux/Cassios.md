###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Cassios
```
┌──(kali㉿kali)-[~/pgplay]
...
``` loginsmb,download`recycler.ser`,file
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Refer to[hacktricks - Deserialization](https://book.hacktricks.xyz/pentesting-web/deserialization?source=post_page-----4686e6fa8df6--------------------------------#ysoserial),downloadFirst`java 11`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `dashboard`Click`check status`canshell,`/home/samantha`get local.txt
```
