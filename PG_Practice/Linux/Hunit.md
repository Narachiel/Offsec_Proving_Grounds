###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Hunit
```
┌──(kali㉿kali)-[~/pgplay]
...
[23:55:46] 500 -  105B  - /error/
``` `http://192.168.208.125:8080/api/`,`/user/`
```json
[
  {
    "string": "/api/",
...
``` `http://192.168.208.125:8080/api/user/`to seeAdmin`dademola/ExplainSlowQuest110`
```json
[
  {
    "login": "rjackson",
...
``` sshlogin,`/home/dademola`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `linpeas.sh`,Click`/etc/crontab.bak``/home/git/.ssh/id_rsa`
```
...
``` Check`/etc/crontab.bak`,findpull,can log ingit
```
[dademola@hunit tmp]$ cat /etc/crontab.bak
*/3 * * * * /root/git-server/backups.sh
*/2 * * * * /root/pull.sh
``` kali`git clone`,`backups.sh`push,get `chmod`
get `chmod`
get `chmod`
get `chmod`
```
