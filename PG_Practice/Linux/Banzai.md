###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Banzai
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `admin`login,canfind8295portdirectory
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.208.56
...
``` Open ncshell.php,get `/home/banzai`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`linpeas.sh`,`config``root/EscalateRaftHubris123`
```
...
define('DBNAME', 'main');
?>
``` then use mysql[Linux Privilege Escalation – Exploiting User-Defined Functions](https://steflan-security.com/linux-privilege-escalation-exploiting-user-defined-functions/),download[edb-1518](https://www.exploit-db.com/exploits/1518?source=post_page-----6cc4d6eea356--------------------------------),`.so`ftpupload ```
┌──(kali㉿kali)-[~/pgplay]
...
# path is /var/www/html/raptor_udf2.so
...
``` Refer to[edb-1518](https://www.exploit-db.com/exploits/1518?source=post_page-----6cc4d6eea356--------------------------------)
```
...
``` can`/etc/passwd`toorroot,/rootget proof.txt
```
