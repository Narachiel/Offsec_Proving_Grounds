###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Fanatastic
```
┌──(kali㉿kali)-[~/pgplay]
...
``` FirstcanFirst use[edb-50581](https://www.exploit-db.com/exploits/50581)FirstLFI,get `/etc/passwd`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use[CVE-2021-43798](https://github.com/jas502n/Grafana-CVE-2021-43798?tab=readme-ov-file)get
...
``` download[AESDecrypt.go](https://github.com/jas502n/Grafana-CVE-2021-43798/blob/main/AESDecrypt.go),First[](https://al1z4deh.medium.com/proving-grounds-fanatastic-b14a6e535e1f) ```
...
``` get `SuperSecureP@ssw0rd`,sshlogin,`/home/sysadmin`obtainlocal.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search[Disk group privilege escalation](https://vk9-sec.com/disk-group-privilege-escalation/),
```
...
``` get root`id_rsa`,sshlogin,/rootpathget proof.txt
```
┌──(kali㉿kali)-[~/pgplay]
└─$ chmod 600 id_rsa
