###### tags: `Offsec` `PG Practice` `Easy` `Linux` # RubyDome
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search[edb-51293](https://www.exploit-db.com/exploits/51293)
```
┌──(kali㉿kali)-[~/pgplay]
...
``` wait for reverse shell,`/home/andrew`obtainlocal.txt
```
python3 -c 'import pty; pty.spawn("/bin/bash")'
andrew@rubydome:~$ cat local.txt
300a8662b196e5c140902a7bc85bde77
``` Check`sudo -l`,`app.rb`canuseroot
```
...
``` First`app.rb`,Refer to[GTFOBins](https://gtfobins.github.io/gtfobins/ruby/#shell),can getroot
```
