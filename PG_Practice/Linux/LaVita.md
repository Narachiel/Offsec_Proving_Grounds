###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # LaVita
```
┌──(kali㉿kali)-[~/pgplay]
...
Finished
===============================================================
``` `http://192.168.176.38/register`,can
```
Name: admin
E-Mail Address: admin@gmail.com
Password: admin123
Confirm Password: admin123
``` logincan`Debug``enable`
```
APP_DEBUG = [ENABLED]
``` Go to `http://192.168.176.38/.git``404 error`,`Laravel 8.4.0`cangoogle exploit,use[CVE-2021-3129](https://github.com/joshuavanderpoll/CVE-2021-3129?source=post_page-----12bfd272e9cf--------------------------------)
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`linpeas.sh`,find`skunk`sudo
```
...
...
``` usepspyCheckcron,find`UID=1001``artisan`
```
...
...
``` `artisan``shell.php`,obtain`shunk`shellCheck`sudo -l`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/composer/#shell),find,use`www-data`useruse,get root,/rootget proof.txt
```
