###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Sorcerer
```
┌──(kali㉿kali)-[~/pgplay]
...
``` downloadzip,max`/home/.ssh``id_rsa`,sshfind,findcanusescp
```
...
``` max`authorized_keys`kali`id_rsa.pub`,usescp,canssh
```
┌──(kali㉿kali)-[~/pgplay/Sorcerer]
└─$ cat authorized_keys                                                   
...
...
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/start-stop-daemon/#suid)get root,get proof.txt in /root,`/home/dennis`get local.txt
```
