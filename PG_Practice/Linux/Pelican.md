###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Pelican
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search[edb-48654](https://www.exploit-db.com/exploits/48654)`java.env script`fieldreverseshell ![Pelican_1.png](picture/Pelican_1.png) Go to `http://192.168.227.98:8080/exhibitor/v1/ui/index.html`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` get,`/home/charles`get local.txt
```
python3 -c 'import pty; pty.spawn("/bin/bash")'
charles@pelican:/opt/zookeeper$ 
charles@pelican:~$ cat local.txt
444e314b2441abf55faa91f9bc1eec11
``` use`sudo -l`,Check[GTFOBins](https://gtfobins.github.io/gtfobins/gcore/#sudo),finduse`pid`
```
...
``` then use `strings`Checkcan getrootpassword,/rootget proof.txt
```
