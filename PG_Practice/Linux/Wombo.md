###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Wombo
```
┌──(kali㉿kali)-[~/pgplay]
...
|     codeName
|_    Unauthorized
``` use[Redis RCE](https://github.com/Ridter/redis-rce)[RedisModules-ExecuteCommand](https://github.com/n0b0dyCN/RedisModules-ExecuteCommand),First`RedisModules-ExecuteCommand`make,`module.so``redis-RCE`
```
┌──(kali㉿kali)-[~/pgplay/RedisModules-ExecuteCommand]
└─$ make
``` `[i]nteractive shell`,findlogin`root`,/rootpathproof.txt
```
