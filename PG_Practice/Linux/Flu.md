###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Flu
```
┌──(kali㉿kali)-[~/pgplay]
...
|     Connection: Close
``` Search[CVE-2022-26134](https://github.com/jbaines-r7/through_the_wire/tree/main)useget Reverseshell,`/home/confluence`get local.txt
```
...
``` use`linpeas.sh`,`/opt/log-backup.sh`,canshellwait for reverse shell
```
...
...
``` wait for reverse shellobtainroot,/rootobtainproof.txt
```
┌──(kali㉿kali)-[~/pgplay]
