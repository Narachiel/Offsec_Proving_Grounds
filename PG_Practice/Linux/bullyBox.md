###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # bullyBox
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`git-dumper`download,canCheck`bb-config.php`
```php
...
...
``` canthen use [CVE-2022-3552](https://github.com/kabir0x23/CVE-2022-3552)login
```
┌──(kali㉿kali)-[~/pgplay]
...
yuki@bullybox:/$
``` `sudo -l`Check,get root,/rootget proof.txt
```
