###### tags: `Offsec` `PG Practice` `Easy` `Linux` # PlanetExpress
```
┌──(kali㉿kali)-[~/pgplay]
...
``` download`192.168.158.205/config/config.yml`,[Pico Plugins](https://github.com/picocms/Pico/tree/master/plugins)
```
...
##
...
#  enabled: true
...
``` Check`http://192.168.158.205/plugins/PicoTest.php` ![PlanetExpress_1.png](picture/PlanetExpress_1.png) can getpath [9000 - Pentesting FastCGI](https://book.hacktricks.xyz/network-services-pentesting/9000-pentesting-fastcgi) path`planetexpress/index.php`
```bash
## fastcgi.sh
#!/bin/bash
...
--><!DOCTYPE html>
...
``` canuse,usereverseshell,`/home/astro`local.txt
```
# change
...
/usr/sbin/relayd
...
``` Check`help`try`/etc/shadow`,error,canCheck`/etc/shadow`
```
