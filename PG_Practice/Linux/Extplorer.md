###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Extplorer
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.214.16/filemanager/`then use `admin/admin`login,uploadshell.phpGo to `192.168.214.16/shell.php`shell
```
┌──(kali㉿kali)-[~/pgplay]
...
www-data@dora:/$
``` `linpeas.sh`,Check`/var/www/html/filemanager/config/.htusers.php`dora's password
```
...
``` get dora's password`doraemon`,dora
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search[Disk group privilege escalation](https://vk9-sec.com/disk-group-privilege-escalation/),`/root/.ssh`id_rsa,shadow
```
dora@dora:/$ df -h
df -h
...
``` canroot's password,root,get proof.txt in /root,`/home/dora`local.txt
```
┌──(kali㉿kali)-[~/pgplay]
