###### tags: `Offsec` `PG Practice` `Hard` `Linux` # Peppo
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use[113 - Pentesting Ident](https://book.hacktricks.xyz/network-services-pentesting/113-pentesting-ident#ident-user-enum)
```
┌──(kali㉿kali)-[~/pgplay]
...
``` then use `eleanor/eleanor`loginssh,`-rbash`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`/bin`,[GTFOBins](https://gtfobins.github.io/gtfobins/ed/#shell)use`ed`canget bash,`PATH`
```
...
``` `/home/eleanor`get local.txt
```
eleanor@peppo:~$ cat local.txt
4efcbfe70e9ae3219ccbbbcd952a40b0
``` Checkid,`docker`,Check[GTFOBins](https://gtfobins.github.io/gtfobins/docker/#shell)
```
eleanor@peppo:~$ id
uid=1000(eleanor) gid=1000(eleanor) groups=1000(eleanor),24(cdrom),25(floppy),29(audio),30(dip),44(video),46(plugdev),108(netdev),999(docker)
