###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Zino
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.169.64:8003/booked/Web/index.php`canthen use `admin/adminadmin`login,Search[CVE-2019-9581](https://github.com/0sunday/Booked-Scheduler-2.7.5-RCE/blob/main/CVE-2019-9581.py)use
```
┌──(kali㉿kali)-[~/pgplay]
...
$
``` Open nc,usereverse command,wait for reverse shell
```
┌──(kali㉿kali)-[~/pgplay]
...
try:
        ...
        print 'ERROR...'
sys.exit(0)
``` Open nc,upload,wait for reverse shell,/rootget proof.txt
```
┌──(kali㉿kali)-[~/pgplay]
