###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # plum
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`192.168.219.28`Click`Administration`,`admin/admin`login
Search[PluXml Blog 5.8.9 Remote Code Execution](https://packetstormsecurity.com/files/176407/PluXml-Blog-5.8.9-Remote-Code-Execution.html),Refer to FirstClick`Static pages > Edit` ![plum_1.png](picture/plum_1.png) phppayload,Click`save`
Open nc,Click`001`page`see`get shell,`/var/www`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` uselinpeas,mail,`/var/spool/mail/www-data`root's password
```
