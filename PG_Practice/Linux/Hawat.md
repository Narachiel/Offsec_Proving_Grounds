###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Hawat
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `http://192.168.233.147:50080/cloud`,canuse`admin/admin`login,log indownload`issuetracker.zip`,Check`/issuetracker/src/main/java/com/issue/tracker/issues/IssueController.java`
```java
...
``` Check`http://192.168.233.147:30455/phpinfo.php`get ```
$_SERVER['DOCUMENT_ROOT'] /srv/http
``` Check`http://192.168.233.147:17445`,use`admin/admin`login,log inCheck`http://192.168.233.147:17445/issue/checkByPriority`useburpsuiteCheck,find`status code`405,`POST`can400
```
## Request
...
``` Refer to[Use SQL Injection to Run OS Commands & Get a Shell](https://null-byte.wonderhowto.com/how-to/use-sql-injection-run-os-commands-get-shell-0191405/)
```
...
## Response
HTTP/1.1 200 
``` Go to `http://192.168.233.147:30455/cmd.php?cmd=whoami`get ```
root
``` uploadshell.php
```
...
``` get root shell,/rootget proof.txt
```
