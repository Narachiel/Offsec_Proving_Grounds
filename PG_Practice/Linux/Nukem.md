###### tags: `Offsec` `PG Practice` `Intermediate` `Linux` # Nukem
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.172.105/``wordpress`,`wpscan`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Search`simple-file-list 4.2.2 exploit`[edb-48979](https://www.exploit-db.com/exploits/48979),downloadpayload
```
...
``` get `http`shell,`/home/commander`get local.txt
```
...
``` `linpeas.sh`,usercommander's password`CommanderKeenVorticons1990`
```
...
``` usercommander,Checkbinary`/usr/bin/dosbox`
```
...
/usr/bin/dosbox
....
``` Check[GTFOBins](https://gtfobins.github.io/gtfobins/dosbox/#sudo),`/etc/sudoers`,get rootget proof.txt in /root
```
