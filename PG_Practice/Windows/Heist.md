###### tags: `Offsec` `PG Practice` `Hard` `Windows` # Heist
```
┌──(kali㉿kali)-[~/pgplay]
...
```
canfindopen`http server`,Searchcankali,`SSRF`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` use`winrm`find,use`enox/california`login,can`C:\Users\enox\Desktop`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `svc_apache``web admins`,can`ReadGMSAPassword` ![Heist_1.png](picture/Heist_1.png) download[Invoke-GMSAPasswordReader.ps1](https://github.com/ricardojba/Invoke-GMSAPasswordReader)
```
...
``` use`hash`login`svc_apache$`,Check`SeRestorePrivilege`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check[SeRestorePrivilege](https://book.hacktricks.xyz/windows-hardening/windows-local-privilege-escalation/privilege-escalation-abusing-tokens#table),download[EnableSeRestorePrivilege.ps1](https://github.com/gtworek/PSBits/blob/master/Misc/EnableSeRestorePrivilege.ps1)
```
...
``` rdp,`win+U`,`C:\Users\Administrator\Desktop`get proof.txt
```
┌──(kali㉿kali)-[~/pgplay]
