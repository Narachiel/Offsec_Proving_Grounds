###### tags: `Offsec` `PG Practice` `Hard` `Windows` # Craft2
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Refer to[edb-44564](https://www.exploit-db.com/exploits/44564)`odt`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ python 44564.py  
...
``` open`responder`,upload`bad.odt``ntlm`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` then use `john`,get `winniethepooh`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ john thecybergeek.ntlm --wordlist=/home/kali/rockyou.txt

winniethepooh    (thecybergeek)
``` then use `CME`Checksmb`share`,loginCheck,upload`s.php`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Go to `http://192.168.211.188/s.php?cmd=whoami`get ```
craft2\apache 
``` upload reverse shell,wait for reverse shell`C:\Users\apache\Desktop`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` then use `Get-ChildItem``xampp`file,Check`C:\xampp\passwords.txt`,finddb`root`can log in
```
...
``` Go to `http://240.0.0.1/phpmyadmin/`then use rootlogin,canuserroot ![Craft2_1.png](picture/Craft2_1.png) probablyQQ ```
