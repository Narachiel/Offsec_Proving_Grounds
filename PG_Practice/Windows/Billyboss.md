###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # Billyboss
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ??,use`nexus/nexus`login,download[edb-49385](https://www.exploit-db.com/exploits/49385),commandStart nc,http serverdownload
```
┌──(kali㉿kali)-[~/pgplay]
...
┌──(kali㉿kali)-[~/pgplay]
└─$ python3 49385.py 
``` wait for reverse shell`C:\Users\nathan\Desktop`get local.txt
```
...
``` `Godpotato`,`C:\Users\Administrator\Desktop`get proof.txt
```
┌──(kali㉿kali)-[~/pgplay]
