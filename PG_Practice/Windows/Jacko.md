###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # Jacko
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `http://192.168.176.66:8082`canuselogin,Refer to[edb-49384](https://www.exploit-db.com/exploits/49384),`SQL statement`First
```sql
...
``` get reverse,`C:\Users\tony\Desktop`get local.txt
```
...
``` `printspoofer`use,use`Godpotato`canuse,`C:\Users\Administrator\Desktop`get proof.txt
```
