###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # DVR4
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check`http://192.168.218.179:8080/Users.html`canUser`Administrator``Viewer`,canSearch[edb-45296](https://www.exploit-db.com/exploits/45296)usecan
```
...
``` `Viewer``id_rsa`,can,then use `notepad++``\n`
```
...
``` canssh,`C:\Users\viewer\Desktop`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
└─$ chmod 600 id_rsa
...
``` Search[CVE-2022-25012](https://github.com/s3l33/CVE-2022-25012),Firstget
```
...
...
[+] Password: 14WatchD0g$
``` useCME,,`winrm`、`impacket-psexec`、`evil-rm`,use`runas`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` nc,reverseshell,Refer to[runas](https://steflan--security-com.translate.goog/windows-privilege-escalation-runas-stored-credentials/?_x_tr_sl=en&_x_tr_tl=zh-TW&_x_tr_hl=zh-TW&_x_tr_pto=sc),get reverseshell
```
┌──(kali㉿kali)-[~/pgplay]
...
whoami
dvr4\administrator
``` `C:\Users\Administrator\Desktop`get proof.txt
```
