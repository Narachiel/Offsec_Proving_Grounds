###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # AuthBy
```
┌──(kali㉿kali)-[~/pgplay]
...
``` ftplogin,can`offsec``admin`,canthen use `admin/admin`login
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.190.46
...
``` then use `admin/admin`can log in,download
```
┌──(kali㉿kali)-[~/pgplay]
└─$ ftp 192.168.190.46
...
``` use`john`can log in`http://192.168.190.46:242`,use`offsec/elite`login
```
┌──(kali㉿kali)-[~/pgplay]
...
elite            (offsec)
``` then use ftpuploadshell.php,use,downloadreverse
```
ftp> put s.php
...
``` `C:\Users\apache\Desktop`get local.txt
```
C:\Users\apache\Desktop>type local.txt
dd73515fb4d098e2d3cd57a50ea9ca64
``` Check`systeminfo`,CheckgooglecanCheck[edb-40564](https://www.exploit-db.com/exploits/40564),use[windows-kernel-exploits
