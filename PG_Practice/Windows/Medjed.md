###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # Medjed
```
┌──(kali㉿kali)-[~/pgplay]
...
``` First`http://192.168.172.127:8000`then use [CVE-2023-24078](https://github.com/overgrowncarrot1/CVE-2023-24078)First,canㄌ
```
┌──(kali㉿kali)-[~/pgplay]
...
``` then use buster`http://192.168.172.127:45332`
```
┌──(kali㉿kali)-[~/pgplay]
...
===============================================================
``` canCheck`http://192.168.172.127:45332/phpinfo.php`,canshell`C:/xampp/htdocs`
```
...
``` `http://192.168.172.127:8000/fs/C/xampp/htdocs/`uploads.phpCheck`192.168.172.127:45332/s.php?cmd=whoami`
```
medjed\jerren
``` uploadshell
```
...
```,`C:\Users\Jerren\Desktop`get local.txt
```
C:\Users\Jerren\Desktop>type local.txt
type local.txt
8a6c4e74ed85aa7e4ff1527a85a919b7
``` [edb-48789](https://www.exploit-db.com/exploits/48789)
```
┌──(kali㉿kali)-[~/pgplay]
...
C:\bd>shutdown /r
``` wait for reverse shellget Administrator,`C:\Users\Administrator\Desktop`get proof.txt
```
