###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # Resourced
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `enum4linux`,get `V.Ventz/HotelCalifornia194!`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` login`Password Audit`,get`\registry\SYSTEM``\Active Directory\ntds.dit`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` CME..get `L.Livingstone/19a3a7550ce8c505c2d46b5e39d6f808`,`impacket-psexec``win-rm`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `C:\Users\L.Livingstone\Desktop`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` downloadbloodhound,Search`L.LIVINGSTONE@RESOURCED.LOCAL`,Click`Node Info -> OUTBOUND OBJECT CONTROL -> First Degree Object Control` ![Resourced_1.png](picture/Resourced_1.png) can`GenericAll`,canuse[rbcd-attack](https://github.com/tothi/rbcd-attack?source=post_page-----50c25c5a23c5--------------------------------) First
```
┌──(kali㉿kali)-[~/pgplay]
...
``` `resourcedc.resourced.local``/etc/hosts`
```
┌──(kali㉿kali)-[~/pgplay]
└─$ sudo nano /etc/hosts

192.168.233.175 resourcedc.resourced.local
``` login,`C:\Users\Administrator\Desktop`get proof.txt
```
┌──(kali㉿kali)-[~/pgplay/rbcd-attack]
