###### tags: `Offsec` `PG Practice` `Intermediate` `Windows` # Hutch
```
┌──(kali㉿kali)-[~/pgplay]
...
``` Refer to[389, 636, 3268, 3269 - Pentesting LDAP](https://book.hacktricks.xyz/network-services-pentesting/pentesting-ldap),can getdomain`hutch.offsec`
```
┌──(kali㉿kali)-[~/pgplay]
...
...
``` use`ldapsearch`,get `:CrabSharkJellyfish192`,`user:fmcsorley`
```
...
``` Refer to[Local Administrator Password Solution (LAPS)](https://ttp.parzival.sh/pentesting/infrastructure/active-directory/local-administrator-password-solution-laps#retrieving-laps-passwords),download,get `administator`'s password`pJDZwUE2}}4a!+`
```
┌──(kali㉿kali)-[~/pgplay]
...
``` loginadministrator,`C:\Users\Administrator\Desktop`get proof.txt,`C:\Users\fmcsorley\Desktop`get local.txt
```
┌──(kali㉿kali)-[~/pgplay]
